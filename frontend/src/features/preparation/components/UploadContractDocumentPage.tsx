...
const DEFAULT_DOCUMENTS: RequiredDocDef[] = [
  { code: "surat_kontrak", name: "Surat Kontrak" },
  { code: "spmk", name: "Surat Perintah Mulai Kerja (SPMK)" },
  { code: "jaminan_pelaksanaan", name: "Jaminan Pelaksanaan" },
  { code: "daftar_personel", name: "Daftar Personel" },
  { code: "struktur_organisasi", name: "Struktur Organisasi Proyek" },
  { code: "kurva_s", name: "Kurva S" },
  { code: "rencana_mutu", name: "Rencana Mutu" },
  { code: "rencana_k3", name: "Rencana K3" },
  { code: "metode_pelaksanaan", name: "Metode Pelaksanaan" },
];

export const UploadDokumenKontrakPage: React.FC = () => {
  ...
  // 1. Fetch Persiapan Detail
  const { data: persiapan, isLoading } = useQuery<PersiapanDetail>({
    queryKey: ["persiapan-detail", id],
    queryFn: () => apiFetch<PersiapanDetail>(`/api/v1/persiapan/${id}`),
    enabled: !!id,
  });

  const allDocDefs = [...DEFAULT_DOCUMENTS, ...customDocs];
  ...
  const handleAddCustomDoc = async (e: React.FormEvent) => {
    ...
    setCustomDocs((prev) => [...prev, { code, name: customDocName.trim(), isCustom: true }]);
  ...
  const docs = persiapan?.documents || [];
  ...
  if (isLoading) {
    return (
      <div className="min-h-[400px] flex items-center justify-center text-slate-500 text-sm">
        Memuat berkas persiapan kontrak...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 w-full font-sans pb-12">
      ...
      <div>
        <button
          type="button"
          onClick={() => navigate("/persiapan_kontrak")}
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] font-semibold text-slate-600">
              Target : <span className="font-bold text-slate-900">{persiapan?.nama || "Kontrak"}</span>
            </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-3.5 w-full">
          ...
          <div className="bg-[#fef2f2] border border-[#fee2e2] rounded-2xl p-3.5 sm:px-4 sm:py-3.5 flex items-center gap-3 min-w-0 w-full shadow-2xs">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[10.5px] sm:text-[11px] font-bold text-[#ef4444] tracking-wider uppercase truncate">BELUM UPLOAD</div>
              <div className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight my-0.5">{belumUploadCount}</div>
          ...
          <div className="bg-[#f0fdf4] border border-[#dcfce7] rounded-2xl p-3.5 sm:px-4 sm:py-3.5 flex items-center gap-3 min-w-0 w-full shadow-2xs">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[10.5px] sm:text-[11px] font-bold text-[#16a34a] tracking-wider uppercase truncate">UPLOAD</div>
              <div className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight my-0.5">{uploadedCount}</div>
          ...
          <div className="bg-[#f0f9ff] border border-[#e0f2fe] rounded-2xl p-3.5 sm:px-4 sm:py-3.5 flex items-center gap-3 min-w-0 w-full shadow-2xs">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[10.5px] sm:text-[11px] font-bold text-[#0284c7] tracking-wider uppercase truncate">TERVERIFIKASI</div>
              <div className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight my-0.5">{verifiedCount}</div>
          ...
          <div className="bg-[#fefce8] border border-[#fef9c3] rounded-2xl p-3.5 sm:px-4 sm:py-3.5 flex items-center gap-3 min-w-0 w-full shadow-2xs">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[10.5px] sm:text-[11px] font-bold text-[#ca8a04] tracking-wider uppercase truncate">BELUM VERIF.</div>
              <div className="text-xl sm:text-2xl font-bold text-slate-900 leading-tight my-0.5">{belumVerifCount}</div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden">
              ...
              {allDocDefs.map((def, index) => {
                ...
                return (
                  <tr key={def.code} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      {isUploaded ? (
                        <div className="flex items-center justify-between p-2.5 bg-emerald-50/70 border border-emerald-200 rounded-xl text-xs">
                          <div className="truncate pr-2">
                            <span className="font-normal text-emerald-900 block truncate">{doc.file_name}</span>
                    ...
                    {/* Tgl Upload */}
                    <td className="py-4 px-4 text-center text-[13px] text-slate-600 font-normal">
                    ...
                    <td className="py-4 px-4 text-center">
                      {isVerified ? (
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">
                          Terverifikasi
                        </span>
                      ) : isUploaded ? (
                        ...
                        <span className="inline-block px-3 py-1 rounded-full text-xs font-normal bg-slate-100 text-slate-500">
                          Belum Diverifikasi
                        </span>
                      )}
                    </td>
                    ...
                    {/* Status */}
                    <td className="py-4 px-4 text-center text-[13px] font-normal">
                      {isVerified ? (
                        <span className="text-emerald-600 font-medium">Terverifikasi</span>
                      ) : isUploaded ? (
                        <span className="text-blue-600 font-medium">Sudah Upload</span>
                      ) : (
                        <span className="text-slate-400 font-normal">Belum Upload</span>
                      )}
                    </td>
        ...
        {/* Bottom Row Info & Add Custom Document Button */}
        <div className="p-4 sm:p-5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-xs font-semibold text-slate-600">
          <div>
            {DEFAULT_DOCUMENTS.length} dokumen wajib + {customDocs.length} tambahan
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 p-5 lg:p-6 shadow-xs space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
          ...
          <div className="space-y-1">
            ...
            <p className="text-slate-500">Dokumen sedang direview oleh pengawas.</p>
          </div>
      ...
      {/* Modal Tambah Dokumen (Tambahan) - Matching Staging Screenshot */}
      {isAddCustomOpen && (
