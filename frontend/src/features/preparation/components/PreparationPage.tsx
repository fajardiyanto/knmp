...
import { usePreparationList, usePreparationMutations } from "../hooks/usePreparation";
...
export const PersiapanPage: React.FC = () => {
  ...
  const { data: persiapanList, isLoading } = usePreparationList(activeTab);
  const { data: knmps } = useKnmpList();
  const { create } = usePreparationMutations();
