...
import { createLaporan, updateLaporan } from "../api";
...
interface LaporanBulananFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  editingItem?: Laporan | null;
  onSuccess?: () => void;
}
...
export const LaporanBulananFormModal: React.FC<LaporanBulananFormModalProps> = ({
  isOpen,
  onClose,
  editingItem,
  onSuccess,
  ...
  useEffect(() => {
    if (editingItem) {
      setPelaksanaanId(editingItem.pelaksanaan_id);
      setReportTitle(editingItem.nama);
      setReportDate(editingItem.tanggal);

      if (editingItem.additional_data) {
        try {
          const parsed =
            typeof editingItem.additional_data === "string"
              ? JSON.parse(editingItem.additional_data)
              : editingItem.additional_data;
      ...
      // Fallback populate from basic model
      setFormData((prev) => ({
        ...prev,
        identitas_acuan: {
          ...prev.identitas_acuan,
          paket_pekerjaan: editingItem.pelaksanaan_name || editingItem.nama,
          rencana_kum_pct: editingItem.rencana_progres_fisik || 0,
          aktual_kum_pct: editingItem.realisasi_progres_fisik || 0,
          deviasi_pct: (editingItem.realisasi_progres_fisik || 0) - (editingItem.rencana_progres_fisik || 0),
        },
      }));
    } else {
  ...
  // Auto-calculate deviasi in Identitas
  useEffect(() => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingItem?.id) {
        return updateLaporan(editingItem.id, payload);
      }
  ...
  // Photo Upload Handler (converts to base64 for instant preview & backend storage)
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5">
        <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-5xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[92vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          {/* Header */}
          <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/50 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-blue-600 text-white flex items-center justify-center font-black text-sm shadow-md shadow-blue-500/20">
                KKP
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">
                    {editingItem ? "Edit Format Laporan Bulanan" : "Input Format Laporan Bulanan Konstruksi"}
                  </h2>
                  <span className="px-2 py-0.5 rounded-full text-[10.5px] font-extrabold bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300">
                    Standar Resmi KKP
                  </span>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Formulir 7 Bagian Resmi Monitoring Kampung Nelayan Merah Putih
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold rounded-xl border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors shadow-2xs"
                title="Lihat Format Cetak 2 Halaman A4"
              >
                <Printer className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                <span>Preview Cetak KKP</span>
              </button>
          ...
          {/* Subheader: Project & Status Selection */}
          <div className="px-6 py-3 bg-blue-50/40 dark:bg-blue-950/20 border-b border-slate-200 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-12 gap-3 items-center text-xs">
            ...
            <div className="sm:col-span-4 space-y-1">
              ...
              <select
                ...
                className="w-full px-3 py-1.5 text-xs font-medium border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div className="sm:col-span-3 space-y-1">
              <label className="font-bold text-slate-700 dark:text-slate-300">Bulan / Tahun</label>
              <input
                type="text"
                value={formData.bulan_tahun}
                onChange={(e) => setFormData({ ...formData, bulan_tahun: e.target.value })}
                placeholder="Contoh: September 2026"
                className="w-full px-3 py-1.5 text-xs border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>

            {/* Bulan Kontrak Ke- */}
            <div className="sm:col-span-2 space-y-1">
              <label className="font-bold text-slate-700 dark:text-slate-300">Bulan Ke-</label>
              <input
                type="text"
                value={formData.bulan_kontrak_ke}
                onChange={(e) => setFormData({ ...formData, bulan_kontrak_ke: e.target.value })}
                placeholder="1"
                className="w-full px-3 py-1.5 text-xs text-center font-bold border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>

            {/* Status Proyek Pill Buttons */}
            <div className="sm:col-span-3 space-y-1">
              <label className="font-bold text-slate-700 dark:text-slate-300">Status Proyek</label>
                ...
                {(["On Track", "Warning", "Critical"] as const).map((st) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-2 py-1.5 rounded-lg text-[11px] transition-all text-center ${colors[st]}`}
                    >
                      {st}
                    </button>
                  );
          ...
          {/* Stepper / Tabs */}
          <div className="px-6 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center gap-2 overflow-x-auto text-xs shrink-0 py-2">
            {[
              { id: "identitas", label: "1. Identitas & Fasilitas", icon: Building2, desc: "Info Kontrak & 10 Fasilitas" },
              { id: "boq", label: "2. Ringkasan & Detail BoQ", icon: FileSpreadsheet, desc: "8 Kelompok & Progres MC" },
              { id: "risiko", label: "3. Risiko & Readiness", icon: ShieldCheck, desc: "6 Aspek Evaluasi K3" },
              { id: "foto", label: "4. Foto & Pengesahan", icon: Camera, desc: "4 Foto & Tanda Tangan" },
            ].map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2.5 px-4 py-2 rounded-xl transition-all font-semibold ${
                    active
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-[10px] text-slate-400 font-normal mt-0.5">{tab.desc}</p>
                  </div>
                </button>
              );
            })}
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xs space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                    ...
                    <span className="text-[11px] text-slate-500">
                      Otomatis sinkron dengan Kontrak & Pelaksanaan
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 text-xs border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 text-xs font-bold border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-blue-600 outline-none focus:ring-1 focus:ring-blue-500"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 text-xs font-bold border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-emerald-600 outline-none focus:ring-1 focus:ring-blue-500"
                      />
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        className={`w-full px-3 py-2 text-xs font-black border border-slate-300 dark:border-slate-700 rounded-lg bg-slate-100 dark:bg-slate-800/80 cursor-not-allowed ${
                  ...
                  <p className="text-[11px] text-slate-400 italic bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-lg border border-slate-100 dark:border-slate-800">
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800/80 font-bold border-b border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300">
                          ...
                          <th className="p-2.5">Fasilitas / Area</th>
                          <th className="p-2.5 w-36 text-center">Lingkup</th>
                          <th className="p-2.5 w-52 text-center">Status Pekerjaan</th>
                          <th className="p-2.5">Catatan / Detail</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {formData.checklist_fasilitas.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                            ...
                            <td className="p-2.5 font-semibold text-slate-800 dark:text-slate-200">{item.fasilitas}</td>
                            ...
                            <td className="p-2.5">
                              <input
                                ...
                                className="w-full px-2.5 py-1 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500"
                              />
                            </td>
                          </tr>
                        ))}
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xs space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                    ...
                    <span className="text-[11px] text-slate-500">
                      Nilai kontrak, bobot, rencana vs realisasi
                    </span>
                  </div>
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800/80 font-bold border-b border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300">
                          ...
                          <th className="p-2.5">Kelompok BoQ</th>
                          <th className="p-2.5 w-32">Nilai Kontrak (Rp)</th>
                          <th className="p-2.5 w-20">Bobot (%)</th>
                          <th className="p-2.5 w-24">Renc. Kum (%)</th>
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {formData.ringkasan_boq.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40 transition-colors">
                            <td className="p-2 text-center font-bold text-slate-400">{item.no || idx + 1}</td>
                            <td className="p-2 font-semibold text-slate-800 dark:text-slate-200">{item.kelompok_boq}</td>
                            <td className="p-2">
                              <input
                                type="number"
                                ...
                                placeholder="0"
                                className="w-full px-2 py-1 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500 font-mono"
                              />
                            </td>
                            <td className="p-2">
                              <input
                                ...
                                placeholder="0"
                                className="w-full px-2 py-1 text-xs text-center border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500 font-semibold"
                              />
                            </td>
                            <td className="p-2">
                              <input
                                ...
                                placeholder="0"
                                className="w-full px-2 py-1 text-xs text-center border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500 font-semibold text-blue-600"
                              />
                            </td>
                            <td className="p-2">
                              <input
                                ...
                                placeholder="0"
                                className="w-full px-2 py-1 text-xs text-center border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500 font-bold text-emerald-600"
                              />
                            </td>
                            ...
                            <td className="p-2">
                              <input
                                ...
                                className="w-full px-2 py-1 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500"
                              />
                            </td>
                          </tr>
                        ))}
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xs space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors"
                    >
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800/80 font-bold border-b border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300">
                          ...
                          <th className="p-2 w-24">Kode BoQ</th>
                          <th className="p-2 w-28">Area</th>
                          <th className="p-2">Uraian Pekerjaan</th>
                          <th className="p-2 w-16">Bobot %</th>
                          <th className="p-2 w-20">Akt. Kum %</th>
                          <th className="p-2 w-32">Nilai Realisasi</th>
                          <th className="p-2 w-20">Termin/MC</th>
                          <th className="p-2 w-16">Deviasi %</th>
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800/80 font-bold border-b border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300">
                          ...
                          <th className="p-2.5 w-44">Aspek Penilaian</th>
                          <th className="p-2.5">Kondisi Bulan Ini</th>
                          <th className="p-2.5">Risiko / Deviasi</th>
                          <th className="p-2.5">Tindak Lanjut / Mitigasi</th>
                          <th className="p-2.5 w-32">PIC / Target</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {formData.matriks_risiko.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                            <td className="p-2 text-center font-bold text-slate-400">{item.no || idx + 1}</td>
                            <td className="p-2 font-bold text-slate-800 dark:text-slate-200">{item.aspek}</td>
                            <td className="p-2">
                              <textarea
                                rows={2}
                                ...
                                className="w-full p-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 resize-none outline-none focus:ring-1 focus:ring-blue-500"
                              />
                            </td>
                            <td className="p-2">
                              <textarea
                                ...
                                className="w-full p-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 resize-none outline-none focus:ring-1 focus:ring-blue-500 text-rose-700 dark:text-rose-400"
                              />
                            </td>
                            <td className="p-2">
                              <textarea
                                ...
                                className="w-full p-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 resize-none outline-none focus:ring-1 focus:ring-blue-500 text-blue-700 dark:text-blue-300"
                              />
                            </td>
                            <td className="p-2">
                              <input
                                ...
                                className="w-full p-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 outline-none focus:ring-1 focus:ring-blue-500 font-medium"
                              />
                            </td>
                          </tr>
                        ))}
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[1, 2, 3, 4].map((slot) => {
                      ...
                      return (
                        <div
                          key={slot}
                          className="border border-slate-200 dark:border-slate-800 rounded-xl p-3.5 bg-slate-50/50 dark:bg-slate-800/40 space-y-2.5"
                        >
                          ...
                          {/* Image Box / Dropzone */}
                          <div className="relative aspect-[16/9] rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden flex flex-col items-center justify-center">
                            {foto.file_url ? (
                              <img
                                src={foto.file_url}
                                alt={`Foto ${slot}`}
                              ...
                              <label className="flex flex-col items-center justify-center cursor-pointer p-4 text-center h-full w-full hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                                <Camera className="w-8 h-8 text-slate-400 mb-1" />
                                <span className="text-[11px] font-bold text-blue-600 dark:text-blue-400">
                                  Upload Foto {slot}
                                </span>
                                <span className="text-[10px] text-slate-400">PNG, JPG s.d 5MB</span>
                                <input
                                  type="file"
                                  accept="image/*"
                                  className="hidden"
                          ...
                          <div>
                            ...
                            <input
                              ...
                              placeholder="Keterangan progres pekerjaan..."
                              className="w-full px-2 py-1 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800"
                            />
                          </div>
                        </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xs space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {/* Dibuat Oleh */}
                    <div className="p-3.5 border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-800/40 space-y-3">
                      <span className="text-[11px] font-black uppercase text-slate-700 dark:text-slate-300 block border-b pb-1">
                        Dibuat oleh: KONTRAKTOR
                      </span>
                      <div className="space-y-1">
                        <label className="text-[10.5px] font-bold text-slate-600 dark:text-slate-400">Nama Lengkap</label>
                        <input
                          type="text"
                          ...
                          className="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10.5px] font-bold text-slate-600 dark:text-slate-400">Tanggal Pengesahan</label>
                        <input
                          type="date"
          ...
          {/* Footer Actions */}
          <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/50 flex items-center justify-between shrink-0">
            <div className="text-xs text-slate-500">
              Bagian aktif: <strong className="text-slate-800 dark:text-slate-200">{activeTab.toUpperCase()}</strong>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all"
              >
                Batal
              </button>
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl transition-all shadow-md shadow-blue-500/20 disabled:opacity-50 cursor-pointer"
              >
                <Save className="w-4 h-4" />
                <span>{saveMutation.isPending ? "Menyimpan..." : editingItem ? "Simpan Perubahan" : "Simpan Laporan"}</span>
              </button>
            </div>
          </div>
        </div>
