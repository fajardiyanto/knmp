...
const DEFAULT_PCM_DOCUMENTS: RequiredDocDef[] = [
  { code: "pemahaman_kontrak", name: "Pemahaman Kontrak" },
  { code: "struktur_organisasi_personel", name: "Struktur Organisasi & Personel" },
  { code: "jadwal_pelaksanaan_kurva_s", name: "Jadwal Pelaksanaan (Kurva S)" },
  { code: "metode_kerja", name: "Metode Kerja" },
  { code: "rencana_mobilisasi", name: "Rencana Mobilisasi" },
  { code: "rmpk_rencana_mutu", name: "RMPK / Rencana Mutu" },
  { code: "rencana_k3", name: "Rencana K3" },
  { code: "rencana_lingkungan", name: "Rencana Pengelolaan Lingkungan" },
  { code: "alur_komunikasi", name: "Alur Komunikasi & Persetujuan" },
  { code: "rencana_subkontraktor", name: "Rencana Subkontraktor" },
  { code: "berita_acara_pcm", name: "Berita Acara PCM" },
];

export const UploadDokumenPCMPage: React.FC = () => {
  ...
  // 1. Fetch PCM Detail
  const { data: pcm, isLoading } = useQuery<PCMDetail>({
    queryKey: ["pcm-detail", id],
    queryFn: () => apiFetch<PCMDetail>(`/api/v1/pcm/${id}`),
    enabled: !!id,
  });

  const allDocDefs = [...DEFAULT_PCM_DOCUMENTS, ...customDocs];

  // Document Upload Mutation
  const uploadMutation = useMutation({
    mutationFn: async ({ category, file }: { category: string; file: File }) => {
      const fd = new FormData();
      fd.append("documentable_type", "pcm");
  ...
  const handleAddCustomDoc = async (e: React.FormEvent) => {
    ...
    const code = "custom_pcm_" + Date.now();
    ...
    <div className="space-y-6 w-full font-sans pb-12">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] font-semibold text-slate-600">
              Target: <span className="font-bold text-slate-900">{pcm?.nama || "PCM"}</span>
            </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden">
        ...
        <div className="p-4 sm:p-5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-xs font-semibold text-slate-600">
          <div>
            {DEFAULT_PCM_DOCUMENTS.length} dokumen wajib + {customDocs.length} tambahan
          </div>
