...
const DEFAULT_LAPANGAN_DOCUMENTS: RequiredDocDef[] = [
  { code: "serah_terima_lahan", name: "Serah Terima Lahan" },
  { code: "dokumentasi_kondisi_awal", name: "Dokumentasi Kondisi Awal" },
  { code: "gambar_kerja", name: "Gambar Kerja" },
  { code: "approval_material", name: "Approval Material" },
  { code: "jadwal_mobilisasi_alat_personel", name: "Jadwal Mobilisasi Alat dan Personel" },
  { code: "daftar_peralatan", name: "Daftar Peralatan" },
  { code: "daftar_tenaga_kerja", name: "Daftar Tenaga Kerja" },
  { code: "pcm_dokumen", name: "PCM" },
  { code: "undangan_pcm", name: "Undangan PCM" },
  { code: "daftar_kehadiran", name: "Daftar Kehadiran" },
  { code: "notulen_pcm", name: "Notulen PCM" },
  { code: "berita_acara_pcm", name: "Berita Acara PCM" },
  { code: "presentasi_kontraktor", name: "Presentasi Kontraktor" },
  { code: "surat_bebas_sengketa", name: "Surat Pernyataan Lahan Bebas Sengketa" },
  { code: "pakta_integritas", name: "Pakta Integritas" },
];

export const UploadDokumenLapanganPage: React.FC = () => {
  ...
  // 1. Fetch Detail Persiapan Lapangan
  const { data: persiapan, isLoading } = useQuery<PersiapanDetail>({
    queryKey: ["persiapan-lapangan-detail", id],
    queryFn: () => apiFetch<PersiapanDetail>(`/api/v1/persiapan/${id}`),
    enabled: !!id,
  });

  const allDocDefs = [...DEFAULT_LAPANGAN_DOCUMENTS, ...customDocs];
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["persiapan-lapangan-detail", id] });
  ...
  const handleAddCustomDoc = async (e: React.FormEvent) => {
    ...
    const code = "custom_lapangan_" + Date.now();
  ...
  if (isLoading) {
    return (
      <div className="min-h-[400px] flex items-center justify-center text-slate-500 text-sm">
        Memuat berkas dokumen persiapan lapangan...
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 w-full font-sans pb-12">
      ...
      <div>
        <button
          type="button"
          onClick={() => navigate("/persiapan_lapangan")}
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] font-semibold text-slate-600">
              Target: <span className="font-bold text-slate-900">{persiapan?.nama || "Persiapan Lapangan"}</span>
            </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden">
        ...
        <div className="p-4 sm:p-5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-xs font-semibold text-slate-600">
          <div>
            {DEFAULT_LAPANGAN_DOCUMENTS.length} dokumen wajib + {customDocs.length} tambahan
          </div>
