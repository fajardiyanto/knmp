...
export const MobilizationPage: React.FC = () => {
  ...
  // Filter calculations
  const safeLapanganList = Array.isArray(lapanganList) ? lapanganList : [];
  const filteredData = safeLapanganList.filter((item) => {
    const matchSearch =
      search === "" ||
    ...
    return matchSearch && matchUser && matchKnmp && matchFile;
  });

  // 4 Metric Card Counts
  const totalDataCount = safeLapanganList.length;
  const docOnlyCount = safeLapanganList.filter(
    (d) => d.documents && d.documents.length > 0 && !d.documents.some((m) => m.mime_type?.startsWith("image/"))
  ).length;
  const imgOnlyCount = safeLapanganList.filter(
    (d) => d.documents && d.documents.some((m) => m.mime_type?.startsWith("image/"))
  ).length;
  const emptyFileCount = safeLapanganList.filter(
  ...
  return (
    <div className="space-y-6 w-full font-sans pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-white p-5 lg:p-6 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div className="space-y-1.5">
            <span className="text-sm font-medium text-slate-500">Total Data Lapangan</span>
            <h4 className="text-2xl lg:text-3xl font-normal text-slate-900 leading-tight">{totalDataCount}</h4>
          </div>
        ...
        <div className="bg-white p-5 lg:p-6 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div className="space-y-1.5">
            ...
            <h4 className="text-2xl lg:text-3xl font-normal text-slate-900 leading-tight">{docOnlyCount}</h4>
          </div>
        ...
        <div className="bg-white p-5 lg:p-6 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div className="space-y-1.5">
            ...
            <h4 className="text-2xl lg:text-3xl font-normal text-slate-900 leading-tight">{imgOnlyCount}</h4>
          </div>
        ...
        <div className="bg-white p-5 lg:p-6 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div className="space-y-1.5">
            ...
            <h4 className="text-2xl lg:text-3xl font-normal text-slate-900 leading-tight">{emptyFileCount}</h4>
          </div>
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-sm text-slate-400">
                    Memuat data persiapan lapangan...
                  </td>
                </tr>
              ) : currentData.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-sm text-slate-400">
                    Tidak ada data persiapan lapangan ditemukan
                  </td>
                </tr>
              ) : (
                currentData.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-4 px-5 text-center">
                      <button
                        type="button"
                        onClick={() => navigate(`/persiapan_lapangan/${item.id}/documents`)}
      ...
      {/* Modal Tambah / Edit Data Persiapan Lapangan - Pixel-perfect matching Screenshot 2 */}
      {isModalOpen && (
          ...
          <div className="bg-white rounded-2xl max-w-[540px] w-full shadow-2xl border border-slate-200 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200/80">
              <h3 className="text-sm font-bold text-slate-800 tracking-tight">
                {editingItem ? "Edit Data Persiapan" : "Tambah Data Persiapan"}
              </h3>
              <button
                type="button"
              ...
              <div className="p-6 space-y-4 text-xs">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    placeholder="Catatan tambahan tentang persiapan ini..."
