...
const DEFAULT_ISSUE_DOCS: RequiredDocDef[] = [
  { no: 1, code: "foto_issue", name: "Foto (max 5)", badge: "max 5" },
];

export const UploadIssueDocumentPage: React.FC = () => {
  ...
  const { data: issue, isLoading } = useQuery<IssueDetail>({
    ...
    queryFn: () => apiFetch<IssueDetail>(`/api/v1/issue/${id}`),
    enabled: !!id,
  });

  const allDocDefs = [...DEFAULT_ISSUE_DOCS, ...customDocs];

  // Document Upload Mutation
  const uploadMutation = useMutation({
    mutationFn: async ({ category, file }: { category: string; file: File }) => {
      const fd = new FormData();
      fd.append("documentable_type", "issue");
    ...
    onSuccess: () => {
      ...
      showAlert({
        ...
        message: "Berkas dokumen issue berhasil diunggah.",
        type: "success",
      });
    },
    onError: (err: any) => {
  ...
  const handleAddCustomDoc = async (e: React.FormEvent) => {
    ...
    const code = `custom_issue_${Date.now()}`;
    const newDocDef = {
    ...
    if (customUploadFile) {
      ...
      try {
        ...
        showAlert({
          title: "Berhasil Diunggah",
          message: "Dokumen issue berhasil diunggah.",
          type: "success",
        });
      } catch (err: any) {
        showAlert({
  ...
  const uploadedCount = allDocDefs.filter((d) => docs.some((doc) => doc.category === d.code || d.code === "foto_issue")).length;
  const belumUploadCount = totalRequired - (docs.length > 0 ? 1 : 0);

  const verifiedCount = docs.filter((d) => d.status === "terverifikasi").length;
  const verifiedPct = docs.length > 0 ? Math.round((verifiedCount / docs.length) * 100) : 0;
  const belumVerifCount = docs.length - verifiedCount;
  const belumVerifPct = docs.length > 0 ? 100 - verifiedPct : 100;

  if (isLoading) {
    return (
      <div className="min-h-[400px] flex items-center justify-center text-slate-500 text-sm">
        Memuat dokumen issue...
      </div>
    );
  }
    ...
    <div className="space-y-6 w-full font-sans pb-12">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] font-semibold text-slate-600">
              Target: <span className="font-bold text-slate-900">{issue?.knmp_name && issue.knmp_name !== "-" ? issue.knmp_name : "-"}</span>
            </p>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden">
          ...
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-slate-50/80 border-b border-slate-200/90 text-xs font-bold text-slate-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="py-4 px-5 min-w-[260px]">Upload File</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {allDocDefs.map((def) => {
                const isPhotoDoc = def.code === "foto_issue";
                const matchingDocs = isPhotoDoc
                  ? docs
                  : docs.filter((d) => d.category === def.code);
                ...
                return (
                  <tr key={def.code} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    {/* Dokumen Name with badge */}
                    <td className="py-4 px-5 font-normal text-slate-900 text-[14px]">
                        ...
                        <div className="space-y-2">
                          <div className="space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                            {matchingDocs.map((doc) => (
                              <div
                                key={doc.id}
                                className="flex items-center justify-between p-2 bg-emerald-50/70 border border-emerald-200 rounded-xl text-xs"
                              >
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      showConfirm({
                                        ...
                                        message: "Apakah Anda yakin ingin menghapus file dokumen issue ini?",
                                        confirmText: "Hapus",
                                        isDestructive: true,
                          ...
                          {matchingDocs.length < 5 && (
                            <label className="flex items-center justify-center p-2 border border-dashed border-[#b6d0ff] bg-[#eef4ff] hover:bg-[#e2edff] rounded-xl cursor-pointer transition-all text-center">
                              <input
                                type="file"
                                className="hidden"
                              ...
                              <span className="text-xs font-semibold text-[#0d6efd]">
                                + Tambah Foto ({matchingDocs.length}/5)
                              </span>
                            </label>
                          )}
                        </div>
                    ...
                    <td className="py-4 px-4 text-center text-[13px] text-slate-600 font-medium">
                      {matchingDocs[0] ? formatDate(matchingDocs[0].created_at) : "-"}
                    </td>
                    ...
                    {/* Tgl Verifikasi */}
                    <td className="py-4 px-4 text-center text-[13px] text-slate-600 font-medium">
                      {matchingDocs[0]?.verified_at ? formatDate(matchingDocs[0].verified_at) : "-"}
                    </td>
                      ...
                      {hasUploaded ? (
                        <div className="flex items-center justify-center gap-2">
                          <button
                            type="button"
                            onClick={() => {
                              const docToToggle = matchingDocs[0];
                              if (docToToggle) {
        ...
        <div className="p-4 sm:p-5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-xs font-semibold text-slate-600">
          <div>
            1 dokumen wajib + {customDocs.length} tambahan
          </div>
