...
import {
  usePembayaranList,
  usePembayaranSummary,
  useTerminStats,
  usePembayaranMutations,
} from "../hooks/usePembayaran";
import { usePersiapanList } from "../../persiapan/hooks/usePersiapan";
...
export const PembayaranPage: React.FC = () => {
  ...
  const { data: list, isLoading } = usePembayaranList();
  const { data: summary } = usePembayaranSummary();
  const { data: terminStats } = useTerminStats();
  const { data: persiapanKontraks } = usePersiapanList("kontrak");
  const { create } = usePembayaranMutations();
