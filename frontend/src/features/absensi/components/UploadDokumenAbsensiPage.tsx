...
const DEFAULT_ABSENSI_DOCS: RequiredDocDef[] = [
  { no: 1, code: "foto_selfie_doc", name: "Foto Selfie Absensi" },
  { no: 2, code: "foto_lokasi_doc", name: "Foto Lokasi / Area Kerja" },
  { no: 3, code: "bukti_kegiatan_doc", name: "Bukti Kegiatan Lapangan" },
  { no: 4, code: "dokumen_harian_doc", name: "Dokumen Pendukung Harian" },
  { no: 5, code: "berkas_tambahan_doc", name: "Berkas Tambahan" },
];

export const UploadDokumenAbsensiPage: React.FC = () => {
  ...
  // 1. Fetch Absensi Detail
  const { data: absensi, isLoading } = useQuery<AbsensiDetail>({
    queryKey: ["absensi-detail", id],
    queryFn: () => apiFetch<AbsensiDetail>(`/api/v1/absensi/${id}`),
    enabled: !!id,
  });

  const allDocDefs = [...DEFAULT_ABSENSI_DOCS, ...customDocs];

  // Document Upload Mutation
  const uploadMutation = useMutation({
    mutationFn: async ({ category, file }: { category: string; file: File }) => {
      const fd = new FormData();
      fd.append("documentable_type", "absensi");
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["absensi-detail", id] });
      queryClient.invalidateQueries({ queryKey: ["absensi-list"] });
      setUploadingCategory(null);
      showAlert({
        title: "Berhasil Diunggah",
        message: "Berkas dokumen absensi berhasil diunggah.",
        type: "success",
      });
    },
    onError: (err: any) => {
  ...
  const handleAddCustomDoc = async (e: React.FormEvent) => {
    ...
    const code = `custom_absensi_${Date.now()}`;
    const newDocDef = {
    ...
    if (customUploadFile) {
      ...
      try {
        ...
        showAlert({
          title: "Berhasil Diunggah",
          message: "Dokumen absensi berhasil diunggah.",
          type: "success",
        });
      } catch (err: any) {
        showAlert({
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[13.5px] font-semibold text-slate-600">
              Target: <span className="font-bold text-slate-900">{absensi?.pelaksanaan_name || "Absensi Lapangan"}</span>
            </p>
                            ...
                            <button
                              ...
                              onClick={() => {
                                showConfirm({
                                  ...
                                  message: "Apakah Anda yakin ingin menghapus file dokumen absensi ini?",
                                  confirmText: "Hapus",
                                  isDestructive: true,
