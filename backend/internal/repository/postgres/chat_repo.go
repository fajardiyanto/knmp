		...
		if convs[i].LastMessageID != nil {
			...
			msgQuery := `
				SELECT m.id, m.conversation_id, m.sender_id, m.message_type, m.content,
				       m.attachment_url, m.attachment_name, m.attachment_size, m.created_at, m.updated_at,
...
func (r *chatRepo) CreateMessage(msg *domain.Message) (*domain.Message, error) {
	...
	`
	if err := tx.QueryRow(insertMsg, msg.ConversationID, msg.SenderID, msg.MessageType, msg.Content, msg.AttachmentURL, msg.AttachmentName, msg.AttachmentSize).Scan(&msg.ID, &msg.CreatedAt, &msg.UpdatedAt); err != nil {
		return nil, err
	}
	...
	// Update sender's last_read_message_id automatically
	updateSenderRead := `
		UPDATE conversation_members
		SET last_read_message_id = $1, updated_at = NOW()
		WHERE conversation_id = $2 AND user_id = $3
	`
	_, _ = tx.Exec(updateSenderRead, msg.ID, msg.ConversationID, msg.SenderID)
	...
	userQuery := `
		...
		WHERE u.id = $1
	`
	if err := r.db.Get(&u, userQuery, msg.SenderID); err == nil {
		msg.SenderName = u.Name
		msg.SenderEmail = u.Email
		if u.RoleName != nil {
			msg.SenderRole = *u.RoleName
		}
	}
...
func (r *chatRepo) MarkConversationAsRead(convID, userID int64) error {
	...
	queryLatest := `SELECT MAX(id) FROM messages WHERE conversation_id = $1`
	if err := r.db.Get(&latestID, queryLatest, convID); err != nil {
		return err
	}
	...
	updateMember := `
		...
		WHERE conversation_id = $2 AND user_id = $3
	`
	_, err := r.db.Exec(updateMember, latestID.Int64, convID, userID)
	return err
}
